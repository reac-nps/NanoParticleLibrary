...
class EnergyCalculator:
    ...
    def mean_absolute_error(self, particle_list, energy_key):
        y_true = [p.get_energy(energy_key) for p in particle_list]
        y_pred = [self.compute_energy(p) for p in particle_list]
        mae = mean_absolute_error(y_true, y_pred)
        print('Mean Absolute error {:.4f} eV'.format(mae))
        return mae

    def root_mean_square_error(self, particle_list, energy_key):
        y_true = [p.get_energy(energy_key) for p in particle_list]
        y_pred = [self.compute_energy(p) for p in particle_list]
        rmse = mean_squared_error(y_true, y_pred, squared=False)
        print('Root Mean Square error {:.4f} eV'.format(rmse))
        return rmse
