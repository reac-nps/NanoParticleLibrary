...
def run_monte_carlo_for_adsorbates(beta, max_steps, start_particle, energy_calculator, n_adsorbates):
    ...
    def run_monte_carlo_ordering_adsorbates(beta, max_steps, start_particle, ordering_energy_calculator, adsorbates_energy_calculato, n_adsorbates):
        from Core.GlobalFeatureClassifier import AdsorptionFeatureVector

        adsorbates_exchange_operator = RandomExchangeOperator(0.5)
        adsorbates_exchange_operator.bind_adsorbates(start_particle, n_adsorbates)

        symbols = start_particle.get_all_symbols()
        adsorbates_energy_key = energy_calculato_adsobates.get_energy_key()
        ...
        energy_calculato_adsobates.compute_energy(start_particle)
        ...
        while no_improvement < max_steps:
            ...
            adsorbates_exchanges = adsorbates_exchange_operator.random_adsorbate_migration(start_particle)
            ordering_exchanges = ordering_exchange_operator.random_exchange(start_particle)
