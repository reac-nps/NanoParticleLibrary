...
def run_monte_carlo_for_adsorbates(beta, max_steps, start_particle, energy_calculator, local_feature_classifier, adsorbate=None):
    energy_key, local_env_calculator, exchange_operator = setup_monte_carlo(start_particle, energy_calculator,
                                                                            local_feature_classifier)
    ...
    initial_sites_atoms = exchange_operator.random_adsorbate_migration(ads.sites_list, number_of_ads, shared_atoms_by_sites)

    particle_energy = start_particle.get_energy(energy_key)

    start_energy = energy_coorection(particle_energy, initial_adsorbed_sites, adsorbate)
    lowest_energy = start_energy
    stablest_adsorption_sites = initial_adsorbed_sites
    accepted_energies = [(lowest_energy, 0, initial_adsorbed_sites)]
    ...
    while no_improvement < max_steps:
        ...
        new_adsorbed_sites = exchange_operator.random_adsorbate_migration(ads.sites_list, number_of_ads, shared_atoms_by_sites)
       
        accepted_particle = copy.deepcopy(start_particle)
        energy_calculator.compute_energy(start_particle)
        new_energy = energy_coorection(start_particle.get_energy(energy_key), new_adsorbed_sites, adsorbate)
        ...
        if np.random.random() < acceptance_rate:
            ...
            # roll back exchanges and make sure features and environments are up-to-date
            start_energy = energy_coorection(start_particle.get_energy(energy_key), initial_adsorbed_sites, adsorbate)
            start_particle.set_energy(energy_key, start_energy)
...
def energy_coorection(energy, sites_occupied, adsorbate):
    correction = 0
    for site_index in sites_occupied:
        for bond in list(adsorbate['site_list'][site_index]):
